...
class SqliteDriver extends AbstractDriver_1.default {
    ...
    async GetAllTables(schema, dbNames, tableNames) {
        ...
        const tableCondition = tableNames.length > 0
            ...
            : "";
        const rows = await this.ExecQuery(`SELECT tbl_name, sql FROM "sqlite_master" WHERE "type" = 'table'  AND name NOT LIKE 'sqlite_%' ${tableCondition}`);
        rows.forEach(val => {
