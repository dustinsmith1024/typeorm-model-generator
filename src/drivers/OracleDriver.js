...
class OracleDriver extends AbstractDriver_1.default {
    constructor() {
        ...
        this.GetAllTablesQuery = async (schema, dbNames, tableNames) => {
            const tableCondition = tableNames.length > 0
                ...
                : "";
            const response = (await this.Connection.execute(`SELECT NULL AS TABLE_SCHEMA, TABLE_NAME, NULL AS DB_NAME FROM all_tables WHERE owner = (select user from dual) ${tableCondition}`)).rows;
            return response;
        };
        try {
    ...
    async CheckIfDBExists(dbName) {
        const x = await this.Connection.execute(`select count(*) as CNT from dba_users where username='${dbName.toUpperCase()}'`);
