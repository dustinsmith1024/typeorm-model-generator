...
WHERE TABLE_TYPE='BASE TABLE' and TABLE_SCHEMA in (${schema}) AND TABLE_CATALOG in (${MssqlDriver.escapeCommaSeparatedList(dbNames)}) ${tableCondition}`)).recordset;
            return response;
        };
    }
    async GetCoulmnsFromEntity(entities, schema, dbNames) {
        const request = new MSSQL.Request(this.Connection);
        const response = (await request.query(`SELECT TABLE_NAME,TABLE_SCHEMA,COLUMN_NAME,COLUMN_DEFAULT,IS_NULLABLE,
    ...
    async GetIndexesFromEntity(entities, schema, dbNames) {
        const request = new MSSQL.Request(this.Connection);
        const response = [];
        await Promise.all(dbNames.split(",").map(async (dbName) => {
        ...
        entities.forEach(ent => {
            const entityIndices = response.filter(filterVal => filterVal.TableName === ent.tscName && filterVal.TableSchema === ent.schema);
