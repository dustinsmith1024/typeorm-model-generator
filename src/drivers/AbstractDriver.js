...
class AbstractDriver {
    ...
    async GetDataFromServer(connectionOptions, generationOptions) {
        ...
        const sqlEscapedSchema = AbstractDriver.escapeCommaSeparatedList(connectionOptions.schemaName);
        dbModel = await this.GetAllTables(sqlEscapedSchema, connectionOptions.databaseName, connectionOptions.skipTables);
        await this.GetCoulmnsFromEntity(dbModel, sqlEscapedSchema, connectionOptions.databaseName);
        await this.GetIndexesFromEntity(dbModel, sqlEscapedSchema, connectionOptions.databaseName);
        AbstractDriver.FindPrimaryColumnsFromIndexes(dbModel);
        dbModel = await this.GetRelations(dbModel, sqlEscapedSchema, connectionOptions.databaseName, generationOptions);
        await this.DisconnectFromServer();
        dbModel = AbstractDriver.FindManyToManyRelations(dbModel);
        return dbModel;
    }
    async GetAllTables(schema, dbNames, tableNames) {
    ...
    // TODO: change name
    static escapeCommaSeparatedList(commaSeparatedList) {
